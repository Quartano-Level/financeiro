...
class ConexosService {
    ...
    // ─── 1. Lista de processos (imp021) ───────────────────────────────────

    async getProcesses(
        filters?: { priCod?: string; priEspRefcliente?: string; dateFrom?: string },
        page = 1,
        pageSize = 100,
        filCod?: number,
    ) {
        boxLog('getProcesses', { filters, page, pageSize });
        await this.ensureSid();

        const filterList: Record<string, any> = { 'priVldStatus#IN': ['1'] };
        if (filters?.priCod) filterList['priCod#EQ'] = Number(filters.priCod);
        if (filters?.priEspRefcliente)
            filterList['priEspRefcliente#LIKE'] = `%${filters.priEspRefcliente}%`;
        if (filters?.dateFrom)
            filterList['priDtaAbertura#GE'] = new Date(
                `${filters.dateFrom}T00:00:00-03:00`,
            ).getTime();
        ...
        const headers = this.defaultHeaders(filCod);
        const url = '/imp021/list';

        try {
            const resp = await this.client.post(url, body, { headers });
            return { rows: resp.data?.rows || [], count: resp.data?.count ?? 0 };
        } catch (err: any) {
            if (err.response?.status === 401) {
                await this.login();
                const retryResp = await this.client.post(url, body, {
                    headers: this.defaultHeaders(filCod),
                });
                return { rows: retryResp.data?.rows || [], count: retryResp.data?.count ?? 0 };
            }
            throw err;
        }
    }

    // ─── 2. Despesas do processo (imp021/DespesasProcesso) ────────────────

    async getDespesasByProcessId(processId: string | number) {
        boxLog('getDespesasByProcessId', { processId });
        await this.ensureSid();
    ...
    // ─── 3. NFs de saida por processo (com297) ────────────────────────────

    async listInvoicesByProcess(priCod: number, pesCod?: number, filCod?: number) {
        if (!priCod) return [];
        await this.ensureSid();
        ...
        if (pesCod) filterList['pesCod#EQ'] = pesCod;
        ...
        try {
            ...
            if (DEBUG_VERBOSE) console.log(`[com297] priCod=${priCod}: ${rows.length} NFs`);
            return rows;
        } catch (err: any) {
            if (err.response?.status === 401) {
                await this.login();
            ...
            console.error(`[com297] Erro NFs priCod=${priCod}:`, err.message);
            return [];
        }
    }

    // ─── 4. NFs de entrada por processo (com298) ─────────────────────────

    async listEntryInvoicesByProcess(priCod: number, filCod?: number) {
        if (!priCod) return [];
        await this.ensureSid();
        ...
        try {
            ...
            const rows = resp.data?.rows || [];
            if (DEBUG_VERBOSE)
                console.log(`[com298] priCod=${priCod}: ${rows.length} NFs de entrada`);
            return rows;
        } catch (err: any) {
            if (err.response?.status === 401) {
                await this.login();
            ...
            console.error(`[com298] Erro NFs entrada priCod=${priCod}:`, err.message);
            return [];
        }
    }

    async getEntryInvoiceProducts(docCod: number) {
        if (!docCod) return [];
        await this.ensureSid();
        ...
        const body = {
            ...
            pageNumber: 1,
            pageSize: 500,
            serviceName: 'com298.comDocProdutos',
            orderList: { orderList: [{ propertyName: 'dprCodSeq', order: 'asc' }] },
        };

        const headers = this.defaultHeaders();
        const url = `/com298/comDocProdutos/list/${docCod}/1`;
        ...
        try {
            ...
            console.error(`[com298] Erro produtos docCod=${docCod}:`, err.message);
            return [];
        }
    }

    // ─── 5. Financeiro a Pagar por processo (com296) ──────────────────────

    async listPayablesByProcess(priCod: number, filCod?: number) {
        if (!priCod) return [];
        await this.ensureSid();
        ...
        const headers = this.defaultHeaders(filCod);
        const url = '/com296/list';

        try {
            const resp = await this.client.post(url, body, { headers });
            const rows = resp.data?.rows || [];
            if (DEBUG_VERBOSE)
                console.log(`[com296] priCod=${priCod}: ${rows.length} titulos a pagar`);
            return rows;
        } catch (err: any) {
            if (err.response?.status === 401) {
                await this.login();
            ...
            console.error(`[com296] Erro payables priCod=${priCod}:`, err.message);
            return [];
        }
    }

    async getPayableProducts(docCod: number) {
        if (!docCod) return [];
        await this.ensureSid();
        ...
        const body = {
            ...
            pageNumber: 1,
            pageSize: 500,
            serviceName: 'com296.comDocProdutos',
            orderList: { orderList: [{ propertyName: 'dprCodSeq', order: 'asc' }] },
        };

        const headers = this.defaultHeaders();
        const url = `/com296/comDocProdutos/list/${docCod}/1`;
        ...
        try {
            ...
            console.error(`[com296] Erro produtos docCod=${docCod}:`, err.message);
            return [];
        }
    }

    // ─── 6. Encargos gerais de uma NF (com017) ───────────────────────────

    async getEncargosGeraisByInvoice(docTip: number, docCod: number, filCod?: number) {
        if (!docCod) return null;
        await this.ensureSid();
        if (!this.usnCod) throw new Error('Conexos usnCod not captured from /login response');

        // ADR-0009: resolve filCod via explicit arg → env override.
        // Throws MissingFilCodError when neither is set (no hardcoded `2`).
        const resolvedFilCod = filCod ?? config.conexos.filCod;
        if (!Number.isFinite(resolvedFilCod)) {
            throw new MissingFilCodError({
                endpoint: `com017/encargosGerais/${docTip}/${docCod}`,
            });
        }

        const headers = {
            ...this.getAuthHeaders(),
            'cnx-filcod': String(resolvedFilCod),
            // PR #19: usnCod captured at runtime from /login response.
            'cnx-usncod': this.usnCod,
            'cnx-datalanguage': 'pt',
            accept: 'application/json, text/plain, */*',
        };

        const url = `/com017/encargosGerais/${docTip}/${docCod}/${resolvedFilCod}/1/1`;

        try {
            const resp = await this.client.get(url, { headers });
            if (DEBUG_VERBOSE)
                console.log(
                    `[com017] docCod=${docCod}: despesas=${resp.data?.despesas?.length || 0}`,
                );
            return resp.data;
        } catch (err: any) {
            if (err.response?.status === 401) {
                await this.login();
                const retryResp = await this.client.get(url, {
                    headers: { ...headers, ...this.getAuthHeaders() },
                });
                return retryResp.data;
            }
            console.error(`[com017] Erro encargos docCod=${docCod}:`, err.message);
            return null;
        }
    }
}
