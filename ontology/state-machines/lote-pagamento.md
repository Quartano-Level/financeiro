...
|---|-----------|----------------|-------|----------|
| L1 | `(novo) → RASCUNHO` | `criarLoteCandidato` (manual) / `formarLotesAutomaticos` (cron) | Abre um lote **RASCUNHO** para **uma** filial (`filCod`); banco/conta opcionais (metadado). Manual: analista abre vazio (`automatico=false`). Cron: `formarLotesAutomaticos` cria já preenchido (`automatico=true`) agrupando títulos a-vencer ≤7d por **filial** (I4), para revisão (internacional fora do escopo — ADR-0020; sem divisão por classe). Ver `actions/sispag/gerenciar-lote-candidato.md` e `actions/sispag/formar-lotes-automaticos.md`. | 2026-07-08 |
